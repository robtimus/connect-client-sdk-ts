...
import { forgeCryptoEngine } from "../../../src/crypto/forge";
import { subtleCryptoEngine } from "../../../src/crypto/SubtleCrypto";

function testCryptoEngine(cryptoEngine: CryptoEngine): void {
  describe("encrypt", () => {
    const payload = {
      clientSessionId: cryptoEngine.randomString(),
      paymentValues: [
        {
          key: "cardNumber",
    ...
    test("can be decrypted", async () => {
      ...
      const encryptedPayload = await cryptoEngine.encrypt(JSON.stringify(payload), keyId, publicKey);
